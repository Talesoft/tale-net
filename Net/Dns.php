...
class Dns
{
    ...
    public static function lookUp($hostName, $type = RecordType::ANY, $withAuthEntries = false, $withAdditionalLookups = false)
    {
        ...
        if (!$result)
            throw new \RuntimeException(
                "Failed to get DNS records for $hostName, maybe there is no DNS available on that host"
            );

        foreach ($result as $record) {

            $type = constant(RecordType::class.'::'.$record['type']);
            switch ($type) {
                case RecordType::A6:
    ...
    public static function lookUpReverse(Address $ipAddress)
    {
        ...
        return self::lookUp($ipAddress->getReverseHostName(), RecordType::PTR);
    }
